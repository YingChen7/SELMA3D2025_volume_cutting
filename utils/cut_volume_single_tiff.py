...
def save_slice(parameters, zdim, region, save_cache=True):
    ...
    for patch in zdim:
      ...
      filehandling.writeNifti(out_path + ".nii.gz", volume, compress=True)
...
def slice_zdim(parameters, zdim, region, save_cache=False):
    ...
    for tiff in tiffs_to_load:
        ...
        for patch in zdim_r:
            ...
            if not os.path.exists(path):
                os.makedirs(path)
            tiff_path = os.path.join(path, f"tiffslice_{tiffs_to_load.index(tiff)}.tif")    
            try:
                tifffile.imwrite(tiff_path, image_cut)
    ...
    print("Sliced tiffs.")
    region = save_slice(parameters, zdim, region, save_cache)
    return region
...
def cut_volume(parameters):
    ...
    patch_size = region['partitioning']['patch_size']
    ...
    n_patches = np.ceil((np.asarray(region['partitioning']['cropping_boundingbox']) - np.asarray(region['partitioning']['cropping_offset']) - np.asarray(patch_overlap))/ (np.asarray(patch_size) - np.asarray(patch_overlap)))
    ...
    print(region['partitioning']['cropping_boundingbox'], region['partitioning']['patch_size'], patch_interval)
    ...
    for patch in region['patches']:
        if not str(patch['id']) in parameters['advanced']['empty_patches']:
            if(os.path.isfile(os.path.join(region['dataset']['localfolder'], 'patchvolume_' + str(patch['id']) + '.nii.gz'))==False):
    ...
    for z_dim in single_tiff_list:
        ...
        before = datetime.datetime.now()
        region = slice_zdim(parameters, z_dim, region, save_cache=parameters['partitioning']['safe_cache'])
        after = datetime.datetime.now()
        delta = after - before
